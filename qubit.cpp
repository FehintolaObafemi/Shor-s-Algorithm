/*Below is the much simpler code for a single qubit. You may find it entertaining
and educational to write a simple driver program for this class to toy with a
single qubit.*/
#include <iostream.h>
#include <math.h>
#include <stdlib.h>
...
class Qubit {
    public:
        ...
        int Measure(); //Returns zero_state = 0 or one_state = 1 in accordance with the probabilities of zero_state and one_state.
        void Dump(); //Prints our zero_state, and one_state, without disturbing anything, this operation has no physical realization, it is only for information and debuging. It should never be used in an algorithm for information.
    ...
    private:
        Complex zero_state; //The probability of finding the Qubit in the zero or all imarinary state. I currently use only the real portion.
        Complex one_state; //The probability of finding the Qubit in the one or all real state. I currently use only the real portion. |zero_state|^2 + |one_state|^2 should always be 1. This notation means z_s * ComplexConjugate(z_s) + o_s * ComplexConjugate(o_s) = 1.
};
...
void Qubit::Dump() {
    cout << "Amplitude of the zero state is " << zero_state.Real() << " +i" << zero_state.Imaginary() << endl << flush;
    cout << "Amplutide of the one state is " << one_state.Real() << " +i" << one_state.Imaginary() << endl << flush;
}
